...
def build_head(head_type,
    ...
    if head_type == 'adaface':
        ...
        head = CenterLoss(embedding_size=embedding_size,
                       classnum=class_num,
                       a=a
                       )
    else:
...
class CosFace(nn.Module):
    ...
    def __init__(self, embedding_size=512, classnum=51332,  s=64., m=0.9):
    ...
    def raw_logit(self, embbedings, norms):
        kernel_norm = l2_norm(self.kernel,axis=0)
        cosine = torch.mm(embbedings,kernel_norm)
...
class Normal(Module):
    def __init__(self, embedding_size=512, classnum=51332):
        ...
        self.fc = nn.Linear(embedding_size, classnum)
...
class CenterLoss(Module):
    def __init__(self, embedding_size=512, classnum=51332,  alpha=0.5):
        
        super(CenterLoss, self).__init__()
        self.classnum = classnum
        self.embedding_size = embedding_size
        #centers
        self.centers = Parameter(torch.randn(classnum, embedding_size))  
        self.alpha = alpha
        self.fc = nn.Linear(embedding_size, classnum)

    def forward(self, embeddings, norm, labels):
        logits = self.fc(embeddings)
        return logits
    ...
    def center_loss(self, embeddings, labels):
        ...
        distmat = torch.pow(embeddings, 2).sum(dim=1, keepdim=True).expand(batch_size, self.classnum) + \
                  torch.pow(self.centers, 2).sum(dim=1, keepdim=True).expand(self.classnum, batch_size).t()
        distmat.addmm_(1, -2, embeddings, self.centers.t())

        classes = torch.arange(self.classnum, device=embeddings.device).long()
        labels = labels.unsqueeze(1).expand(batch_size, self.classnum)
        mask = labels.eq(classes.expand(batch_size, self.classnum))
